...
Azure Red Hat OpenShift (ARO) clusters store log data inside the cluster by default. Understanding metrics and logs is critical in successfully running your cluster. Included with ARO is the OpenShift Cluster Logging Operator, which is intended to simplify log management and analysis within an ARO cluster, offering centralized log collection, powerful search capabilities, visualization tools, and integration with other other Azure systems like https://azure.microsoft.com/en-us/products/storage/blobs[Azure Blob Storage,window=_blank].

In this section of the workshop, we'll configure ARO to forward logs and metrics to Azure Blob Storage and view them using Grafana.
...
== Configure Metrics and Log Forwarding to Azure Blob Storage

. First, let's create our Azure Blob Storage account. To do so, run the following command::
+
[source,sh,role=execute]
----
...
. Now let's save these values in our `.workshoprc` file in case we get disconnected:
+
[source,sh,role=execute]
----
cat << EOF >> ~/.workshoprc
...
# Azure storage
export AZR_STORAGE_ACCOUNT_NAME="storage${GUID}"
export ARO_LOCATION=$(az aro show --resource-group openenv-${GUID} --name aro-cluster-${GUID} --query location -o tsv)
export AZR_STORAGE_KEY=\$(az storage account keys list --resource-group "openenv-${GUID}" -n "${AZR_STORAGE_ACCOUNT_NAME}" --query "[0].value" -o tsv)
...
oc label namespaces openshift-logging openshift.io/cluster-monitoring="true" --overwrite=true

oc create ns openshift-operators-redhat
oc label namespaces openshift-operators-redhat openshift.io/cluster-monitoring="true" --overwrite=true
...
NAME: clf-operators
LAST DEPLOYED: Tue Nov  4 20:34:38 2025
...
. Now that the operators have been successfully installed, let's use a helm chart to deploy Grafana and forward metrics to Azure Blob Storage. To do so, run the following command:
+
[source,sh,role=execute]
----
...
[source,sh,role=execute]
----
oc -n custom-logging get route aro-thanos-af-grafana-cr-route \
...
echo "export STORAGE_CLASS=${STORAGE_CLASS}" >> .workshoprc
echo ${STORAGE_CLASS}
----
+
.Sample Output
...
. Next, let's use another helm chart to deploy forward logs to Azure Blob Storage. To do so, run the following command:
+
[source,sh,role=execute]
----
helm upgrade -n custom-logging aro-clf-blob \
 --install mobb/aro-clf-blob --version 0.2.37 \
...
. Wait until loki has been successfully deployed. Run the following command:
+
[source,sh,role=execute]
----
oc -n openshift-logging rollout status deploy logging-loki-distributor
oc -n openshift-logging rollout status deploy logging-loki-gateway
oc -n openshift-logging rollout status deploy logging-loki-querier
oc -n openshift-logging rollout status deploy logging-loki-query-frontend
----
+
.Sample Output
[source,text,options=nowrap]
----
deployment "logging-loki-distributor" successfully rolled out
deployment "logging-loki-gateway" successfully rolled out
deployment "logging-loki-querier" successfully rolled out
deployment "logging-loki-query-frontend" successfully rolled out
----

== View the Metrics and Logs in Grafana

Now that the metrics and log forwarding are forwarding to Azure Blob Storage, let's view them in Grafana.
...
+
This you can safely ignore, the reason is because we have only given permission to see logs in the `custom-logging` namespace and Grafana tries to access API endpoints it does not have permissions to use. When setting up the label selector you will also see a few warning about access, that is again because Grafana tries to read all values from the backend but it's limited in access.

. The data source `Loki (Application)` is preselected in the dropdown for "Data source", if not, select it.
+
image::loki-datasource-selector-apps.png[]
+
In the labels field first enter
+
[source,text,options=nowrap]
----
log_type
----
+
Then click on the `Use custom value`, then click `= Equals` and enter
+
[source,text,options=nowrap]
----
application
----
+
Then press the enter key (Grafana tries to ). For the second label, enter:
+
[source,text,options=nowrap]
----
...
----
+
Complete the label setup by pressing the enter key.
+
. Click the "Show logs" button to view application logs from loki.
+
image::loki-grafana-app-logs.png[]

. Now select the data source `Loki (Infrastructure)` in the dropdown for "Data source":
+
image::loki-datasource-selector-infra.png[]
+
In the labels field first enter
+
[source,text,options=nowrap]
...
Complete the label setup by pressing the enter key.
+
. Click the "Show logs" button to view infrastructure logs from loki.
+
image::loki-grafana-infra-logs.png[]

== Enabling Custom Metrics

In order to display metrics from your own applications you need to enable custom metrics. We already did this as part of the `aro-thanos-af` helm chart earlier.

. Let's confirm these settings by checking the cluster-monitoring-config ConfigMap object. You can see here how we have enabled the `enableUserWorkload: true` flag and setup `remoteWrite` to `thanos`.
+
[source,sh,role=execute]
----
...
. We can also check that the User workload monitoring has started up (wait until the output below matches what you see):
+
[source,sh,role=execute]
----
...
== View the Metrics and Logs in the OpenShift console

In addition to viewing logs in Grafana, you can also view logs stored in Azure Blob Storage directly inside the OpenShift console with the help of the OpenShift logging UI plugin. The logging UI plugin is provided by the Red Hat OpenShift Cluster Observability Operator that we installed as part of the aro-clf-blob helm chart.

- To view metrics, to go the "Observe" left hand menu and then click on "Metrics".
- To view logs, to go the "Observe" left hand menu and then click on "Logs" and select appropriate filter
...
* View your stored logs in the OpenShift Console
