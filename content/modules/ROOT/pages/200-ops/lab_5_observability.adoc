...
[source,sh,role=execute]
----
helm repo add mobb https://scorpiion.github.io/helm-charts/
...
[source,sh,role=execute]
----
oc -n custom-logging rollout status deploy grafana-operator-controller-manager-v5
----
...
. Wait until thanos and the grafana operator has been successfully deployed. Run the following command:
+
[source,sh,role=execute]
----
oc -n custom-logging rollout status deploy aro-thanos-af-grafana-cr-deployment
oc -n custom-logging rollout status deploy thanos-querier
----
+
.Sample Output
[source,text,options=nowrap]
