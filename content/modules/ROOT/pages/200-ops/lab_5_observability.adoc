...
helm upgrade -n "custom-logging" aro-thanos-af \
  --install mobb/aro-thanos-af --version 0.7.1 \
...
helm upgrade -n custom-logging aro-clf-blob \
 --install mobb/aro-clf-blob --version 0.2.25 \
...
. You can now browse to Observe -> Logs in the console to see your logs arriving from LokiStack
